...
5. **语音输入** — 支持麦克风录音输入，基于本地 Whisper ASR 转文字，无需额外 API
6. **反馈报告** — 从学术深度、表达清晰度、真实性风险三个维度评分，附带漏洞分析和改进建议
...
### 后续方向
1. **真实面试案例库** — 邀请成功保研的学长学姐提供真实面试案例，提炼高频追问模式，融入 Prompt，让模拟更贴近真实场景         
2. **导师画像与方向匹配** — 整合目标院校导师的研究方向、论文、面试风格等信息，让提问更具针对性                              
3. **保研时间规划倒计时** — 添加夏令营/预推免关键节点倒计时组件，增加紧迫感，引导学生尽早准备                              
4. **真实面试记录** — 允许学生记录自己在真实面试中被问到的问题作为补充材料用于后续模拟训练   
